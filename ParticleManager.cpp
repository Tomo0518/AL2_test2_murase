...
void ParticleManager::LoadParams() {
	...
	debris.acceleration = { 0.0f, 0.0f };     // ★追加
	debris.emitRange = { 20.0f, 20.0f };      // ★追加：少し散らす
	...
	debris.rotationSpeedMin = -0.1f;          // ★追加：回転しながら落ちる
	debris.rotationSpeedMax = 0.1f;           // ★追加
	...
	hit.acceleration = { 0.0f, 0.0f };        // ★追加
	hit.emitRange = { 10.0f, 10.0f };         // ★追加
	hit.sizeMin = 27.0f;
	hit.sizeMax = 43.0f;
	hit.scaleStart = 0.8f;
	hit.scaleEnd = 0.2f;
	...
	hit.rotationSpeedMin = 0.0f;              // ★追加
	hit.rotationSpeedMax = 0.0f;              // ★追加
...
void ParticleManager::Draw(const Camera2D& camera) {
	...
	for (auto it = params_.begin(); it != params_.end(); ++it) {
		...
		for (auto& p : particles_) {
			...
			// 描画サイズを取得（0なら1フレーム分のサイズを使用）
			float baseSize = p.GetDrawSize();
			if (baseSize <= 0.0f) {
				baseSize = static_cast<float>(srcW);  // アニメーション考慮
			}
